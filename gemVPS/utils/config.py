...
class Settings(BaseModel):
    """
    Pydantic model for loading and validating all environment variables for the
    application. This provides a single, reliable source of truth for all
    configuration.
    """
    ...
    # Websocket URLs are not yet supported in Pydantic's HttpUrl
    ALCHEMY_WEBSOCKET_URL: str
    ...
    POSTGRES_USER: str = Field(
        "trader",
        description="Default PostgreSQL username",
    )
    ...
    POSTGRES_HOST: str = Field(
        "database",
        description="The service name from docker-compose",
    )
    ...
    ENABLE_AUTO_TRADING: bool = Field(
        False,
        description="Master switch for automated trade execution",
    )
...
def load_config() -> Settings:
    """
    Loads environment variables from the .env file and validates them using the
    Settings model.
    ...
    # Pydantic will automatically read the environment variables and validate
    # them
    return Settings()
