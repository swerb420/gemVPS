...
class ChartGenerator:
    ...
    def __init__(self):
        ...
        """
        try:
            # For a production system, you would place font files (e.g., from
            # Google Fonts) in a 'fonts' directory and reference them here.
            ...
            logger.warning(
                "Custom font files not found. Falling back to default font."
            )
            self.title_font = ImageFont.load_default()
            self.body_font = ImageFont.load_default()
            self.label_font = ImageFont.load_default()

    async def create_status_dashboard(
        self, data: Dict[str, Any]
    ) -> io.BytesIO:
        """
        Creates a comprehensive status dashboard image from various data
        points.

        Args:
            data (Dict[str, Any]):
                A dictionary containing all the data to be displayed.
        ...
        d.text(
            (col1_x, 110),
            f"CPU Usage: {data.get('cpu_usage', 0):.1f}%",
            font=self.label_font,
            fill='#FFFFFF',
        )
        d.text(
            (col1_x, 135),
            f"RAM Usage: {data.get('ram_usage', 0):.0f} MB",
            font=self.label_font,
            fill='#FFFFFF',
        )
        ...
        d.text(
            (col1_x, 220),
            f"ETH Gas Price: {data.get('gas_price', 0):.1f} Gwei",
            font=self.label_font,
            fill='#FFFFFF',
        )
        d.text(
            (col1_x, 245),
            f"Dominant Narrative: {data.get('dominant_narrative', 'N/A')}",
            font=self.label_font,
            fill='#FFFFFF',
        )
        ...
        d.text(
            (col2_x, 220),
            f"Composite Strength: {signal_strength:.2f}",
            font=self.label_font,
            fill=signal_color,
        )
        ...
        d.text(
            (20, 370),
            "Generated by Elite Trading Bot",
            font=self.label_font,
            fill='#566573',
        )
